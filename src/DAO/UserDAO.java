...
public class UserDAO {
    ...
    public int toggleTrangThaiUser(int id) {
        int kt = -1;
        String queryget ="SELECT trangThai FROM USERS WHERE idUser =? ";
        String query = "UPDATE USERS SET trangThai = ? WHERE idUser = ?";
    ...
    public int getIDUserLogin(){ 
        int id= 0 ;
        String query = "SELECT idUserFROM USERS WHERE trangThai=2";
        try(Connection conn = getConnection();
                PreparedStatement stmt = conn.prepareStatement(query);
                ResultSet rs =stmt.executeQuery();){ 
            if(rs.next()){ 
    ...
    public String getTenUserByID(int id){ 
        String query = "SELECT ten FROM USERS WHERE idUser = ? ";
        try(Connection conn = getConnection();
                PreparedStatement stmt = conn.prepareStatement(query)){ 
            stmt.setInt(1,id);
            ResultSet rs = stmt.executeQuery();
    ...
    public String getTenTaiKhoanUserByID(int id){ 
        String query = "SELECT taiKhoan FROM USERS WHERE idUser = ? ";
        try(Connection conn = getConnection();
                PreparedStatement stmt = conn.prepareStatement(query)){ 
            stmt.setInt(1,id);
            ResultSet rs = stmt.executeQuery();
