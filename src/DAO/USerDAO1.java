...
public class USerDAO1 {
    ...
    public List<User> getAllUsers() {
        ...
        String sql = "SELECT u.idUser, u.taiKhoan, u.ten, u.matKhau, r.tenVT, u.trangThai " +
    ...
    public void suaUser(User u, String maVT) {
        String sql = "UPDATE USERS SET ten=?, matKhau=?, trangThai=?, maVT=? WHERE idUser=?";
        try (Connection conn = getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public void voHieuHoaUser(int id) {
        String sql = "UPDATE USERS SET trangThai = 0 WHERE idUser = ?";
        try (Connection conn = getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, id);
            ps.executeUpdate();
    ...
    public User getUserById(int id) {
        User user = null;
        String sql = "SELECT u.*, r.tenVT FROM USERS u JOIN ROLES r ON u.maVT = r.maVT WHERE u.idUser = ?";
        try (Connection conn = getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
